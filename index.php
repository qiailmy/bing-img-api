...
// 默认备用壁纸
$default_wallpaper = [
    'urlbase' => '/th?id=OHR.SnowyMountains_ZH-CN9869777329',
    'startdate' => date('Ymd'),
    'copyright' => '雪山风光 | 默认备用壁纸',
    'title' => 'Bing 每日壁纸'
];

// 获取 Bing 壁纸数据：使用本地缓存，外部 API 抖动时快速回退
function get_bing_wallpaper($idx = 0, $n = 8) {
    $cache_file = __DIR__ . '/bing-wallpapers-cache.json';
    $cache_ttl = 3600;
    if (is_file($cache_file) && time() - filemtime($cache_file) < $cache_ttl) {
        $cached = json_decode((string)file_get_contents($cache_file), true);
        if (!empty($cached['images'])) return array_slice($cached['images'], 0, $n);
    }
    $api_url = "https://www.bing.com/HPImageArchive.aspx?format=js&idx={$idx}&n={$n}&mkt=zh-CN";
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_USERAGENT => 'Mozilla/5.0 BingWallpaperHome/2.0'
    ]);
    $response = curl_exec($ch);
    $http_code = (int)curl_getinfo($ch, CURLINFO_HTTP_CODE);
    curl_close($ch);
    $data = ($http_code === 200 && $response) ? json_decode($response, true) : null;
    if (!empty($data['images'])) {
        ...
        return array_slice($data['images'], 0, $n);
    }
...
// 获取近8天壁纸
$wallpapers = get_bing_wallpaper(0, 8);

// 如果获取失败，用默认数据填充
if (empty($wallpapers)) {
    $wallpapers = array_fill(0, 8, $default_wallpaper);
}

$today_wallpaper = $wallpapers[0] ?? $default_wallpaper;

// 格式化日期
function format_date($date_str) {
    if (strlen($date_str) < 8) return $date_str;
...
// 获取图片URL（Bing标准格式：urlbase + _宽x高.jpg）
function get_image_url($url_base, $w = 1920, $h = 1080) {
    // 标准尺寸直接用 Bing 原生尺寸后缀
    $standard_sizes = ['1920x1080', '1080x1920', '1366x768', '800x480', '400x240'];
    $size = "{$w}x{$h}";
    
    if (in_array($size, $standard_sizes)) {
        return "https://www.bing.com{$url_base}_{$size}.jpg";
    }
    // 自定义尺寸用UHD缩放
    return "https://www.bing.com{$url_base}_UHD.jpg&w={$w}&h={$h}&rs=1&c=4";
}
?>
<!DOCTYPE html>
<html lang="zh-CN">
...
<body>
    <div class="container">
        ...
        <div class="today-wallpaper">
            <img id="todayImg" src="<?php echo get_image_url($today_wallpaper['urlbase'], 1920, 1080); ?>" alt="今日必应壁纸" onerror="this.style.display='none'">
            <a href="<?php echo $site_url; ?>/download.php?url=<?php echo urlencode('https://www.bing.com' . $today_wallpaper['urlbase'] . '_UHD.jpg'); ?>" class="download-btn" id="todayDownload" download>
                <span>⬇</span> 下载原图
            </a>
            <div class="wallpaper-info">
                <div class="title">今日必应壁纸</div>
                <div class="copyright" id="copyrightText"><?php echo format_date($today_wallpaper['startdate']); ?> | <?php echo htmlspecialchars($today_wallpaper['copyright']); ?></div>
            </div>
        </div>
        ...
        <div class="weekly-wallpapers">
            ...
            <div class="weekly-scroll" id="weeklyScroll">
                <?php foreach ($wallpapers as $index => $wp): ?>
                <div class="weekly-item <?php echo $index === 0 ? 'active' : ''; ?>" data-index="<?php echo $index; ?>">
                    <img src="<?php echo get_image_url($wp['urlbase'], 400, 240); ?>" alt="壁纸" onerror="this.style.background='#6366f1';this.src=''">
                    <div class="date"><?php echo $index === 0 ? '今天' : ($index === 1 ? '昨天' : substr($wp['startdate'], 4, 2) . '/' . substr($wp['startdate'], 6, 2)); ?></div>
                </div>
                <?php endforeach; ?>
            </div>
        ...
        <div class="api-section">
            ...
            <div class="api-grid">
                <div class="api-card">
                    ...
                    <div class="code-box">
                        <code class="api-url" data-base="<?php echo $site_url; ?>/auto_302.php"><?php echo $site_url; ?>/auto_302.php</code>
                ...
                <div class="api-card">
                    ...
                    <div class="code-box">
                        <code class="api-url" data-base="<?php echo $site_url; ?>/auto.php"><?php echo $site_url; ?>/auto.php</code>
                        <button class="copy-btn" onclick="copyCode(this)">复制</button>
                    </div>
                ...
                <div class="api-card">
                    ...
                    <div class="code-box">
                        <code class="api-url" data-base="<?php echo $site_url; ?>/random.php"><?php echo $site_url; ?>/random.php</code>
                        <button class="copy-btn" onclick="copyCode(this)">复制</button>
                    </div>
        ...
        <div class="api-section">
            ...
            <div class="api-grid">
                <div class="api-card">
                    ...
                    <div class="code-box">
                        <code class="api-url" data-base="<?php echo $site_url; ?>/m_302.php"><?php echo $site_url; ?>/m_302.php</code>
                        <button class="copy-btn" onclick="copyCode(this)">复制</button>
                    </div>
                ...
                <div class="api-card">
                    ...
                    <div class="code-box">
                        <code class="api-url" data-base="<?php echo $site_url; ?>/1920x1080_302.php"><?php echo $site_url; ?>/1920x1080_302.php</code>
                        <button class="copy-btn" onclick="copyCode(this)">复制</button>
                    </div>
                ...
                <div class="api-card">
                    ...
                    <div class="code-box">
                        <code class="api-url" data-base="<?php echo $site_url; ?>/1366x768_302.php"><?php echo $site_url; ?>/1366x768_302.php</code>
                        <button class="copy-btn" onclick="copyCode(this)">复制</button>
                    </div>
                ...
                <div class="api-card">
                    ...
                    <div class="code-box">
                        <code class="api-url" data-base="<?php echo $site_url; ?>/func.php?info=1"><?php echo $site_url; ?>/func.php?info=1</code>
                        <button class="copy-btn" onclick="copyCode(this)">复制</button>
                    </div>
    ...
    <script>
        ...
        var wallpaperData = <?php echo json_encode(array_map(function($wp) {
            return [
                'urlbase' => $wp['urlbase'],
                'startdate' => $wp['startdate'],
                'copyright' => $wp['copyright']
            ];
        }, $wallpapers), JSON_UNESCAPED_UNICODE); ?>;
